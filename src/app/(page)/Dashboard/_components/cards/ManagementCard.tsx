...
export default function ManagementCard({
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <div className="relative flex h-[40vh] flex-col justify-between rounded-lg border  border-border bg-card px-5 py-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-gray-300 hover:shadow-md hover:shadow-gray-200">
        <h3 className="text-[15px] font-semibold text-foreground">{title}</h3>
        <div className="space-y-2">
          <p className="text-sm leading-relaxed text-muted-foreground">{description}</p>
          {description2 && <p className="text-sm leading-relaxed text-muted-foreground">{description2}</p>}
        </div>
