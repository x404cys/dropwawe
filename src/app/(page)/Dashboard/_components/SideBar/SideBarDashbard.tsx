...
const navItems = [
  { label: 'الرئيسية', path: '/Dashboard', icon: HiHome },
  { label: 'المنتجات', path: '/Dashboard/ProductManagment', icon: HiCube },
  { label: 'اضافة منتج', path: '/Dashboard/ProductManagment/add-product', icon: HiPlusCircle },
  { label: 'الطلبات', path: '/Dashboard/OrderTrackingPage', icon: HiShoppingCart },
  { label: 'العوائد', path: '/Dashboard/profit', icon: HiCash },
  { label: 'الموردين', path: '/Dashboard/products-dropwave', icon: HiUserGroup },
  { label: 'الاعدادات', path: '/Dashboard/setting/store', icon: HiCog },
];
...
export default function SideBarDashboard({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div dir="rtl" className="hidden h-screen flex-col bg-gray-50 md:flex">
      <header className="flex h-16 items-center justify-between border bg-white px-6">
        <Image src="/Dropwave-logo.svg" alt="logo" width={130} height={100} />
        ...
        {openUserMenu && (
          <div
            dir="rtl"
            className="absolute top-15 right-3 left-8 z-50 w-56 rounded-lg bg-white shadow-lg ring-1 ring-gray-200"
          >
        ...
        {openNotifications && (
          <div
            dir="rtl"
            className="absolute top-14 left-72 z-50 w-80 max-w-sm rounded-lg bg-white shadow-lg ring-1 ring-gray-300"
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="hidden h-full w-56 overflow-y-auto border-l px-4 py-6">
          <ul className="space-y-2">
            {navItems.map(item => {
              const isActive = pathname === item.path && item.path !== '/Dashboard';
                ...
                <li key={item.path}>
                  <Link
                    ...
                    className={`flex items-center gap-3 px-4 py-3 transition-all ${
                      isActive
                        ? 'bg-muted -translate-x-3 shadow-blue-200/30'
                        : 'text-black duration-300 hover:-translate-x-2 hover:bg-gray-100'
                    }`}
                  >
        ...
        <main className="scrollbar-none flex-1 overflow-y-auto scroll-smooth bg-gray-50 p-2">
          {children}
        </main>
      </div>
    </div>
