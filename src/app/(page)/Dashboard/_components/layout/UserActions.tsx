...
export default function UserActions() {
  ...
  const menuAlignClass = dir === 'rtl' ? 'right-0' : 'left-0';
  const notificationsAlignClass = dir === 'rtl' ? 'left-0' : 'right-0';
    ...
    >
      <div className="flex items-center gap-3">
        <div className="relative">
          ...
          {openUserMenu && (
            <div
              className={`border-border bg-card absolute ${menuAlignClass} z-50 mt-2 w-44 rounded-xl border shadow-lg`}
            >
              <ul className="flex flex-col p-2 text-sm">
                <li>
                  <button
                    ...
                    className={`hover:bg-muted w-full rounded-lg px-3 py-2 text-xs ${textAlignClass}`}
                  >
                    {t.nav.profile}
                  </button>
                </li>
            ...
            <div className="relative">
              ...
              >
                <span className="max-w-[110px] truncate">
                  {currentStore?.name || t.profile.chooseStore}
                </span>
              ...
              {openStoreMenu && (
                <div
                  className={`border-border bg-card absolute ${menuAlignClass} z-50 mt-2 w-52 rounded-xl border shadow-lg`}
                >
                  <ul className="flex flex-col p-2 text-sm">
                    {stores.map(store => (
                      <li key={store.id}>
                        <button
                          ...
                          className={`hover:bg-muted flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm ${
          ...
          <AnimatePresence>
            {openNotifications && (
              <motion.div
                ...
                animate="visible"
                exit="exit"
                className={`border-border/60 bg-card absolute ${notificationsAlignClass} z-50 mt-1.5 w-80 overflow-hidden rounded-2xl border shadow-xl shadow-black/10`}
              >
