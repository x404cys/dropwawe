      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {items.map(item => (
            <article
              key={item.id}
              className="rounded-2xl border border-gray-200 bg-white p-4 transition-all duration-200 ease-in-out hover:shadow-sm"
            >
              <Quote className="mb-3 h-5 w-5" style={{ color: `${colors.primary}25` }} />
              <p className="mb-4 text-xs leading-relaxed text-gray-600">{item.text}</p>
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  {item.role ? <p className="text-[9px] text-gray-500">{item.role}</p> : null}
                </div>
