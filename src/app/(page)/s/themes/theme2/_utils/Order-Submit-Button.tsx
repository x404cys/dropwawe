...
export default function OrderSubmitButton({
  ...
  return (
    <div className="w-72 font-light">
      <motion.button
        whileTap={{ scale: 0.97 }}
        disabled={loading}
        onClick={handleSubmit}
        className={`w-full rounded-none border-2 bg-black px-4 py-2 text-sm font-light text-white transition-all duration-500 ${
          loading
            ? 'cursor-not-allowed bg-black text-gray-500'
            : 'cursor-pointer border-gray-800 bg-black font-light text-gray-900'
        }`}
      >
        {loading ? (
          ...
          >
            {buttonText} <PiHandCoinsLight className="rotate-[-10deg] text-xl" />
          </motion.div>
        )}
      </motion.button>
